...
function create_tables($db, $prefix = '') {
    //读取SQL文件
    $sql = file_get_contents(MODULE_PATH . 'Data/createTables.sql');
    $sql = str_replace("\r", "\n", $sql);
    $sql = explode(";\n", $sql);
...
/**
*  创建视图
*  @param return $db 数据库连接资源
*/
function create_views($db,$prefix = '') {
    $sql = file_get_contents(MODULE_PATH. 'Data/createViews.sql');
    $sql = str_replace("\r", "\n", $sql);
    $sql = explode(";\n", $sql);
    ...
    foreach ($sql as $value) {
        $value = trim($value);
        if (empty($value)) continue;
        if (substr($value, 0, 22) == 'CREATE OR REPLACE VIEW') {
            $name = preg_replace("/^CREATE TABLE `(\w+)` .*/s", "\\1", $value);
            $msg  = "创建数据表{$name}";
            if (false !== $db->execute($value)) {
                show_msg($msg . '...成功');
