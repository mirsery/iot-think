...
class IndexController extends Controller{
    ...
    protected function _initialize() {
        ...
        if (Storage::has('./Data/install.lock')) {
            $this->error('已经成功安装了本系统，请不要重复安装!', U('Admin/Index/index'));
        } else if ($_SERVER[ENV_PRE.'DEV_MODE'] === 'true') {
            $this->error('系统处于开发模式，无需安装！', U('Admin/Index/index'));
        }
    }
    ...
    public function step3($db = null) {
        ...
        $db["DB_TYPE"] = I("DB_TYPE");
        $db["DB_HOST"] = I("DB_HOST");
        $db["DB_NAME"] = I("DB_NAME"); 
        $db["DB_USER"] = I("DB_USER");
        $db["DB_PWD"] = I("DB_PWD");
        $db["DB_PREFIX"] = I("DB_PREFIX");
    ...
    public function step4() {
        ...
        create_views($db_instance,$db_config['DB_PREFIX']);

        //创建配置文件
        $conf = write_config($db_config, $auth);

        //根据加密字符串更新admin密码的加密结果
        $new_admin_password = user_md5('welcome1','root');
        $sql = <<<SQL
        UPDATE `{$db_config["DB_PREFIX"]}sys_user` SET `password`='{$new_admin_password}' WHERE `id` = 1;
SQL;
